...
export const BattleActions = (props) => {
        ...
        if (hp > 0 && hpOpponent > 0) {
            if (moveOpponetDetail) {
                ...
                if (defese < attackOpponent) {
                    ...
                    setPowerDefense(attackOpponent * 0.80)
                }
                ...
                if (moveOpponetDetail.flavor_text_entries[0].flavor_text === '') {
                    ...
                    setOpponentTextAttack(moveOpponetDetail.flavor_text_entries[0].flavor_text)
                }
                if (defeseOpponent < attack) {
                    setPowerOpponetDefense(attack - defeseOpponent)
                } else {
                    setPowerOpponetDefense(attack * 0.80)
                }
    ...
    return (
        <div>
            <h1>{pokemon.name}</h1>
            {pokemon.sprites && <img src={pokemon.sprites.versions['generation-v']['black-white'].animated.back_default}
                                     alt={pokemon.name}/>}
            {pokemon.types[0] && <p>{pokemon.types[0].type.name}</p>}
            {pokemon.moves[0] &&
            <button onClick={() => moveName(pokemon.moves[0].move.name)}>{pokemon.moves[0].move.name}</button>}
            {pokemon.moves[1] &&
            <button onClick={() => moveName(pokemon.moves[1].move.name)}>{pokemon.moves[1].move.name}</button>}
            {pokemon.moves[2] &&
            <button onClick={() => moveName(pokemon.moves[2].move.name)}>{pokemon.moves[2].move.name}</button>}
            ...
            {pokeData ? (
                <div>
                    <h1>{pokeData.name}</h1>
                    {pokeData.sprites &&
                    <img src={pokeData.sprites.versions['generation-v']['black-white'].animated.front_default}
                         alt={pokeData.name}/>}
